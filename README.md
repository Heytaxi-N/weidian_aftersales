...
| 5 | **D 待填退货单号** | 买家版 | 关联到的退货单号，每笔一条，每轮都推 |
| 6 | **☀️ 早报**（仅 09:00） | 卖家版 | 昨日推送 / 已处理 / 仍未处理 / 今日新增 |
...
- 按**客户手机号**关联：买家版订单收件人手机（`getOrderDetailForApp.buyerInfo.telephone`）== 卖家版 `待商家收货` 退款的客户手机（`buyer_phone`/`receiver_phone`）
- 命中则带上卖家版的退货单号 → 提醒店主复制去买家版填写
- 手机号归一化后比较（去空格/横线/前导 86）；每轮都推，不去重不限额
...
> 规格：藏青色;L
> 收件人：王某 / 198****xxxx
> 退货单号：`21878xxxxxxxxxxxx`（复制此单号去买家版填写）
```
...
8. **双 webhook 隔离**：卖家版(A/A2/B/概览/告警)和买家版(C/D)走两个群机器人，节流按 webhook URL 分桶，互不阻塞。
...
```
95e0921 D 消息去掉退款编号行
f022567 场景 D 接线 + 推送（后半）：退货单号关联提醒上线
1dc7db3 场景 D 数据层 + 匹配逻辑（前半）：客户手机号关联
91396da 买家版退款列表改用 frontRefundList API，覆盖全部店铺
687d1fc 新增场景 C：买家版退款临期推送（独立 webhook）
f90a2e4 修复诊断脚本两个 bug：候选池统计 stale + 配额耗尽未识别
9f9736b 清理：移除 evaluate 的 already_pushed 死参数
